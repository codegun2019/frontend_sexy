      ...
      {cardData.map((data) => (
        <Card key={data.id} className="w-full lg:w-full xl:w-1/1 hover:w-100 transition-transform transform hover:-translate-y-1 hover:shadow-lg hover:scale-105">
          ...
          <CardBody className="text-center">
            <Typography variant="h4" color="blue-gray" className="mb-2 font-noto-700">
              {data.name}
            </Typography>
            <Typography color="blue-gray" className="font-medium font-noto-400" textGradient>
              @{data.title}
            </Typography>
            <div className="flex justify-between w-full mt-4 mb-2">
              <div className="flex flex-col items-center">
                <span className="font-noto-700 text-lg">{data.post}</span>
                <span className="text-sm font-noto-400 text-gray-500">Posts</span>
              </div>
              <div className="flex flex-col items-center">
                <span className="font-noto-700 text-lg">{data.followes}</span>
                <span className="text-sm font-noto-400 text-gray-500">Followers</span>
              </div>
              <div className="flex flex-col items-center">
                <span className="font-noto-700 text-lg">{data.followings}</span>
